...
class TestWorkerCapabilityPublishing:
    ...
    def test_idle_count_initialization(self, mock_broadcaster):
    ...
    def test_idle_count_decrement_on_job_claim(self, mock_broadcaster):
    ...
    def test_idle_count_increment_on_job_complete(self, mock_broadcaster):
    ...
    def test_heartbeat_publishes_periodically(self, mock_broadcaster):
        """Test that heartbeat publishes capabilities at intervals."""
        from src.worker import ComputeWorker
        ...
        with patch("src.worker.get_broadcaster", return_value=mock_broadcaster):
            ...
            # Initial check (before interval)
            worker._check_and_publish_heartbeat()
            assert not mock_broadcaster.publish_retained.called

            # Simulate time passing beyond heartbeat interval
            worker.last_heartbeat_time = time.time() - 35  # Past 30s interval
